...
def build_assertions(bundle: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for evidence in bundle.get("evidence", []):
        ...
        if all(isinstance(value, str) and value for value in (subject_type, subject_id, evidence_id)):
            evidence_by_subject.setdefault((subject_type, subject_id), []).append(evidence_id)

    assertions: list[dict[str, Any]] = []
    for collection_name, (subject_type, default_predicate) in _COLLECTION_SUBJECT.items():
